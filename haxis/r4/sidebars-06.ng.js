	...
	HAX.defaults = {
		...
		direction: 13, directionString: '0', 
		objCount: 300,
		lineCount: 100, lineField: 14, lineEnd: 14, lineLength: 10,
...
*/
	...
	css.innerHTML = 
		...
		'div.sidebar { background-color: #eee;  padding: 0 10px 20px 10px; position: absolute; top: 40px;;} ' +
	'';
	...
	var stats = document.body.appendChild(document.createElement( 'div' ) );
	...
	settingsMenu.style.cssText = 'right: 450px; ' ;
...
function initText() {
		...
		settings.style.cssText = 'display: none; right: 450px; width: 300px;' ;
		...
		readout.style.cssText = 'display: none; right: 20px; width: 390px;' ;
		readout.innerHTML = '<p>Enterprise.</p>';
		...
		settings.innerHTML += '<hr><div id="settings2">';
	}	
	...
	function updateSbr() {
		...
		buildSelect2( HAX.geneva, 0, td);

		tr = table.appendChild( document.createElement( 'tr' ) );
		td = tr.appendChild( document.createElement( 'td' ) );
		td.innerHTML = 'San Francisco:';
		td = tr.appendChild( document.createElement( 'td' ) );
		buildSelect2( HAX.sanFrancisco, 0, td);

		tr = table.appendChild( document.createElement( 'tr' ) );
		td = tr.appendChild( document.createElement( 'td' ) );
		td.innerHTML = 'Zurich:';
		td = tr.appendChild( document.createElement( 'td' ) );
		buildSelect2( HAX.zurich, 0, td);
...
console.log( item );	
	...
	if ( dataPlay !== true ) { return; }
		resetPermalink();
/*		
		var id = event.srcElement.selectedIndex;
		var h = event.srcElement.obj;
		
		HAX.folder = h.folder;
		HAX.fname = h.fields[ id ];
		HAX.title = h.fieldsTitles[ id ];
		HAX.x = h.x;
		HAX.y = h.y;
		HAX.z = h.z;
console.log('hi', event, event.srcElement.selectedIndex, event.srcElement.obj);		
*/		
